-- The target is Carto, meaning copy and paste and execute this once 
-- but you can also call in a test environment like
--    export PGUSER=localuser
--    export PGPASSWORD=localpass
--    export PGDATABASE=localdb
--    psql -f function_deploy.sql
drop function if exists open_street_node;

CREATE OR REPLACE FUNCTION open_street_node(
  text
) 
...
drop function if exists open_street_radius;

CREATE OR REPLACE FUNCTION open_street_radius(
  integer,
  integer,
  integer
) 
...
drop function if exists open_street_segment;

CREATE OR REPLACE FUNCTION open_street_segment(
  text
) 
